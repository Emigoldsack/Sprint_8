...
class Loan(APIView):
    ...
    def delete(self, request, customer_id): ## BORRA EL PRESTAMO DE UN CLIENTE
        prestamo = Prestamo.objects.filter(customer_id=customer_id).first() ## PREGUNTAR A NICO 
        if prestamo:
            serializer = PrestamoSerializer(prestamo)
            prestamo.delete()
            return Response(serializer.data, status=status.HTTP_200_OK)
...
class BranchLoan(APIView): ## TRAE LOS PRESTAMOS DE UN SUCURSAL (en teoria)
    def get(self, request,branch_id):
        brachloan = Prestamo.objects.filter(branch_id=branch_id).first() ## PREGUNTAR A NICO 
